...
from utils.calculations import calculate_beam_moment_capacity, calculate_shear_capacity, calculate_deflection_check
...
def generate_rectangular_beam_dxf(b, d, dia_bottom, n_bottom, dia_top, n_top, 
    ...
    # Setup layers as per LISP logic
    doc = create_dxf_header(doc, f"Rectangular Beam B-{beam_num}")
    
    # Extract scale factor
    scale_factor = int(scale.split(':')[1])
    
    # Drawing parameters (matching LISP logic)
    dim_text_height = 3 * scale_factor
    
    # Beam corner points (bottom-left origin) - matching LISP PT1, PT2, PT3, PT4
    pt1 = (0, 0)  # Bottom left
    pt2 = (b, 0)  # Bottom right  
    pt3 = (b, d)  # Top right
    pt4 = (0, d)  # Top left
    
    # Draw beam outline (PLINE command in LISP)
    msp.add_lwpolyline([pt1, pt2, pt3, pt4, pt1], dxfattribs={'layer': 'OUTLINE'})
    
    # Calculate reinforcement positions (matching LISP DDASB, DDAST)
    cover = 25  # Standard cover
    dda_sb = cover + dia_bottom/2  # Bottom bar center distance
    dda_st = cover + dia_top/2     # Top bar center distance
    
    # Bottom reinforcement positions (matching LISP PT5, PTC1)
    pt5 = (dda_sb, dda_sb)
    ptc1 = (pt5[0] + 0.25 * dia_bottom, pt5[1])  # Leader point
    
    # Top reinforcement positions (matching LISP PT6, PTC2)
    pt6 = (dda_st, d - dda_st)
    ptc2 = (pt6[0] + 0.25 * dia_top, pt6[1])  # Leader point
    
    # Calculate spacing (matching LISP SPACINGB, SPACINGT)
    if n_bottom > 1:
    ...
    # Draw bottom bars (DONUT command in LISP)
    for i in range(n_bottom):
        x_pos = pt5[0] + i * spacing_bottom
        center = (x_pos, pt5[1])
        msp.add_circle(center, dia_bottom/2, dxfattribs={'layer': 'REINFORCEMENT'})
    
    # Draw top bars (DONUT command in LISP)
    for i in range(n_top):
        x_pos = pt6[0] + i * spacing_top
        center = (x_pos, pt6[1])
        msp.add_circle(center, dia_top/2, dxfattribs={'layer': 'REINFORCEMENT'})
    
    # Draw stirrup outline (matching LISP stirrup logic)
    stirrup_clear = cover
    stirrup_pts = [
    ...
    # Add dimensions (matching LISP DIM commands)
    # Width dimension (HORIZONTAL DIM)
    pt_d1 = (0, -1.2 * dim_text_height)
    add_dimensions(msp, pt1, pt2, dim_line_y_offset=-1.2 * dim_text_height, text=str(b))
    
    # Height dimension (VERTICAL DIM)
    pt_d2 = (-1.2 * dim_text_height, 0)
    add_dimensions(msp, pt1, pt4, dim_line_y_offset=-1.2 * dim_text_height, text=str(d))
    
    # Add leader lines and notes (matching LISP logic)
    # Bottom bar leader line
    pt_d3 = pt5
    pt_d4 = (pt_d3[0], pt_d3[1] + 0.2 * d)
    pt_d5 = (pt_d4[0] + b, pt_d4[1])
    msp.add_line(pt_d3, pt_d4, dxfattribs={'layer': 'LEADERS'})
    msp.add_line(pt_d4, pt_d5, dxfattribs={'layer': 'LEADERS'})
    
    # Top bar leader line
    pt_d6 = pt6
    pt_d7 = (pt_d6[0], pt_d6[1] - 0.2 * d)
    pt_d8 = (pt_d7[0] + b, pt_d7[1])
    msp.add_line(pt_d6, pt_d7, dxfattribs={'layer': 'LEADERS'})
    msp.add_line(pt_d7, pt_d8, dxfattribs={'layer': 'LEADERS'})
    
    # Stirrup leader line
    pt_d9 = (b - 25, d/2)
    pt_d10 = (pt_d9[0] + 25 + dda_sb, pt_d9[1])
    msp.add_line(pt_d9, pt_d10, dxfattribs={'layer': 'LEADERS'})
    
    # Add reinforcement notes (matching LISP TEXT commands)
    text_height = 0.1 * d
    
    # Bottom steel note
    bottom_note = f"{n_bottom}NOS.{dia_bottom}MM DIA.TOR BARS"
    msp.add_text(bottom_note, 
                dxfattribs={'height': text_height, 'insert': pt_d5})
    
    # Top steel note
    top_note = f"{n_top}NOS.{dia_top}MM DIA.TOR BARS" 
    msp.add_text(top_note,
                dxfattribs={'height': text_height, 'insert': pt_d8})
    
    # Stirrup note
    stirrup_note = f"{stirrup_dia}MM DIA.TWO LEGGED STIRRUPS @{stirrup_spacing}MM C/C."
    msp.add_text(stirrup_note,
                dxfattribs={'height': text_height, 'insert': pt_d10})
    
    # Beam identification (matching LISP beam title)
    pt_d11 = (0, -4 * dim_text_height)
    beam_title = f"BEAM NO.B-{beam_num}"
    title_height = 0.125 * d
    msp.add_text(beam_title,
                dxfattribs={'height': title_height, 'insert': pt_d11})
