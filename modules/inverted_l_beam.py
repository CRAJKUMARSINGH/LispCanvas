...
def page_inverted_l_beam():
    st.title("Inverted L-Beam Designer")
    ...
    with st.sidebar:
        ...
        bf = st.number_input("Flange Width (mm)", min_value=300, value=900, step=50)
        tf = st.number_input("Flange Thickness (mm)", min_value=50, value=150, step=10)
        
        # Web Dimensions
        st.subheader("Web Dimensions")
        bw = st.number_input("Web Width (mm)", min_value=100, value=250, step=10)
        d = st.number_input("Effective Depth (mm)", min_value=200, value=450, step=10)
        ...
        flange_position = st.radio("Flange Position", ["Bottom Left", "Bottom Right"])
        ...
        # Loads
        st.subheader("Loads")
        dead_load = st.number_input("Dead Load (kN/m)", min_value=1.0, value=12.0, step=0.5)
    ...
    if st.button("Generate DXF Drawing"):
        dxf_bytes = generate_inverted_l_beam_dxf(bf, tf, bw, D, d, main_bars_dia, num_bars, 
                                              stirrup_dia, stirrup_spacing, flange_position)
        
        # Download button
        st.download_button(
            label="Download Inverted L-Beam DXF",
            data=dxf_bytes,
            file_name=f"inverted_l_beam_{int(bf)}x{int(D)}_{flange_position.replace(' ', '_').lower()}.dxf",
...
def generate_inverted_l_beam_dxf(bf, tf, bw, D, d, main_bars_dia, num_bars, stirrup_dia, stirrup_spacing, flange_position):
    """Generate DXF file for Inverted L-Beam section."""
    ...
    # Determine if flange is on left or right
    is_left_flange = "Left" in flange_position
    
    # Draw outline (inverted L-shape)
    if is_left_flange:
        points = [
            (0, 0),  # Bottom left
            (bf, 0),  # Bottom right of flange
            (bf, tf),  # Top right of flange
            (bw, tf),  # Start of web right
            (bw, D),  # Top right of web
            (0, D),  # Top left of web
            (0, 0)  # Back to start
        ]
    else:  # Right flange
        points = [
            (0, 0),  # Bottom left
            (bw, 0),  # Bottom right of web
            (bw, tf),  # Top right of web
            (bw + bf, tf),  # Top right of flange
            (bw + bf, 0),  # Bottom right of flange
            (bw, 0),  # Back to web
            (0, 0)  # Back to start
        ]
    ...
    for i in range(num_stirrups):
        ...
        if is_left_flange:
            points = [
                (stirrup_dia/2, y),
                (bw - stirrup_dia/2, y),
                (bw - stirrup_dia/2, y + stirrup_dia),
                (stirrup_dia/2, y + stirrup_dia),
                (stirrup_dia/2, y)
            ]
        else:  # Right flange
            points = [
    ...
    if is_left_flange:
        ...
        add_dim((0, 0), (0, D), -100, f"{D}")  # Total depth
        add_dim((0, tf), (bf, tf), -150, f"{tf}")  # Flange thickness
        add_dim((0, tf), (bw, tf), 50, f"{bw}")  # Web width
    else:  # Right flange
        add_dim((0, 0), (bw + bf, 0), -50, f"{bw + bf}")  # Total width
        add_dim((0, 0), (0, D), -100, f"{D}")  # Total depth
        add_dim((bw, 0), (bw + bf, 0), -150, f"{bf}")  # Flange width
        add_dim((0, tf), (bw, tf), 50, f"{bw}")  # Web width
